...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#file_inbox").change(function(){
	        if($("#file_inbox").val()!= '')
	        {
	            mostrarImagenInbox(this);
	        }
        ...
        $('#txt_des_msj').keypress(function(e){
            if(e.keyCode == 13)
            {
                enviar_mensaje_inbox();
            }
        });
...
function enviar_mensaje_inbox()
{
    ...
    if($("#txt_des_msj").html() == '' & $("#file_inbox").val() == '')
    {
        return false;   
    }
    ...
    var archivos    = document.getElementById("file_inbox");
    var archivo     = archivos.files; 
    var mensaje     = $("#txt_des_msj").html();
    ...
    data.append('file_inbox',archivo[0]);
...
function mostrarImagenInbox(input) 
{
 if (input.files && input.files[0]) {
  var reader = new FileReader();
  reader.onload = function (e) {
   $('#img_inbox').attr('src', e.target.result);
   $("#img_inbox").show();
  }
...
function iniciar_conversacion(id_usuario,avatar,usuario)
{
	if($("#chat_" + id_usuario).length > 0 )
	{
        ...
        $("#usuario_chat").attr("href","u/"+usuario.substring(1));
        ...
        $("#chat_amigos").attr("class","tab-pane fade ")
        $("#chat_prin").attr("class","tab-pane fade active in");
        
        $("#li_chat").attr("class","active");
        $("#li_amigos").attr("class","");
...
<ul class="nav nav-tabs">
    <li id="li_chat" class="active">
        <a aria-expanded="true" href="#chat_prin" data-toggle="tab">
            <img src="img/message-32.png" style="width:16px;" />
            Conversaci&oacute;n
        </a>
    </li>
    <li id="li_amigos">
        <a aria-expanded="true" href="#chat_amigos" data-toggle="tab">
            <img src="img/users.png" style="width:16px;" />
            Amigos
            <span id="sp_not_glo" style="display:none"><img src="img/alert.png" /></span>
        </a>
    </li>  
...
<div id="myTabContent" class="tab-content">
    ...
    <div class="tab-pane fade " id="chat_amigos">   
        <br><br>
                  ...
                  <?
                    if(count($usuarios)>0)
                    {
                        foreach ($usuarios as $rec)
                        {
                            ...
                            ?>
                            <a href="javascript:void(0)" class="list-group-item usuario_chat text-info" data-id-usuario="<?=$rec['id_usuario'];?>" data-nombre-usuario="@<?=$rec['nombre_usuario'].' '.$rec['apellidos'];?>" data-avatar="<?=$rec['avatar'];?>">
    ...
    <div class="tab-pane fade active in" id="chat_prin">   
        <br>
        <div class="">

            <div id="caja_mensajes" class="panel panel-default" style="display:none;margin-bottom: 0px;">
                <div class="panel-heading">
                    <img  id="avatar_chat" src="" class="w16" />
                    <a id="usuario_chat" class="text-info">@Usuarios</a>
                                        ...
                                        <input class="file-data" type="hidden" name="media_data_empty" value="">
                                            <div class="multi-photo-data-container hidden"></div>
                                                <label class="t1-label" style="cursor:pointer;margin-bottom:0px;font-weight:normal;">
                                                    ...
                                                    <span id="im_f"></span>
                                                    <input type="file" name="file_inbox" id="file_inbox" class="file-input" accept="image/*" size ="50" title="Solo se permite subir imagenes" />
                                                </label>
                                        <div class="swf-container"></div>
                                    </div>
                                </a>
                                <a href="#myModa4" role="button" class="btn-emojis"   data-id-div="#txt_des_msj"  data-toggle="modal" style="cursor:pointer;margin-right:10px;" ><img src="img/Emoticon.png" class="w20"/></a>

                                <img id="img_inbox" style="width:100px" />                        
                                <a name="btn_enviar_mensaje" id="btn_enviar_mensaje" class="btn btn-sm btn-success">Enviar</a>
                            </div>
                          </div>
